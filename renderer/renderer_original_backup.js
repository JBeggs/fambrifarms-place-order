...
const ORDERS_LIST_ENDPOINT = BACKEND_API_URL ? `${BACKEND_API_URL.replace(/\/$/, '')}/api/orders/` : '';
const ORDER_DETAIL_ENDPOINT = BACKEND_API_URL ? `${BACKEND_API_URL.replace(/\/$/, '')}/api/orders/` : '';
const ORDER_DELETE_ENDPOINT = BACKEND_API_URL ? `${BACKEND_API_URL.replace(/\/$/, '')}/api/orders/` : '';
...
async function loadProducts() {
  ...
  try {
    ...
    const data = await response.json();
    if (data.results) {
      // Handle paginated response - load all pages
      products = [...data.results];
      let nextUrl = data.next;
      
      while (nextUrl) {
        console.log('[renderer] Loading next page:', nextUrl);
        const nextResponse = await fetchWithTimeout(nextUrl);
        if (!nextResponse.ok) break;
        
        const nextData = await nextResponse.json();
        if (nextData.results) {
          products.push(...nextData.results);
...
async function loadOrders() {
  ...
  try {
    console.log('[renderer] Loading orders from:', ORDERS_LIST_ENDPOINT);
    const response = await fetchWithTimeout(ORDERS_LIST_ENDPOINT);
...
async function deleteOrder(orderId) {
  ...
  try {
    ...
    const response = await fetchWithTimeout(`${ORDER_DELETE_ENDPOINT}${orderId}/`, {
      method: 'DELETE'
    });
...
async function getOrderDetails(orderId) {
  ...
  try {
    ...
    const response = await fetchWithTimeout(`${ORDER_DETAIL_ENDPOINT}${orderId}/`);
...
function showPanel(which) {
  ...
  if (!panelOrders) {
    console.warn('[renderer] Orders panel not found');
    return;
  }
  if (!panelDebug) {
  ...
  if (which === 'orders') {
    ...
    // Load orders when switching to orders panel
    loadOrders().catch(err => console.error('[renderer] Failed to load orders:', err));
  } else if (which === 'debug') {
...
function initializeApp() {
  ...
  if (statusFilterEl) {
    statusFilterEl.addEventListener('change', renderOrdersList);
  }
  
  if (dateFilterEl) {
    dateFilterEl.addEventListener('change', renderOrdersList);
  }
  
  if (customerFilterEl) {
    customerFilterEl.addEventListener('input', renderOrdersList);
  }
